...
def iperfilinglinux(filename, commandline) :
    ...
    ws.merge_cells(start_row = lines - 1, start_column = 2, end_row = lines - 1, end_column = 9)
...
def iperfilinglinux_udp(filename, commandline) :
    ...
    while(1):
        ...
        if v.__contains__("sec"):
            ...
            fwrite.write('-,-,-,-,-,-,-,-\n')
            fwrite.write('interval,sec,transfer,bytes,bitrate,bits/sec,jitter(ms),remarks\n')
            break        
        elif v == "\n":
            break
        elif v == "":
